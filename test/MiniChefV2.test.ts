...
describe("MiniChefV2", function () {
  before(async function () {
    await prepare(this, ['MiniChefV2', 'AceToken', 'ERC20Mock', 'RewarderMock', 'RewarderBrokenMock'])
    await deploy(this, [
  ...
  beforeEach(async function () {
    ...
    await deploy(this,
      ...
      ['chef', this.MiniChefV2, [this.ace.address]],
    ...
    await this.ace.mint(this.chef.address, getBigNumber(10000))
    await this.lp.approve(this.chef.address, getBigNumber(10))
    await this.chef.setAcePerSecond("10000000000000000")
  ...
  describe("PendingAce", function() {
    it("PendingAce should equal ExpectedAce", async function () {
      ...
      let expectedAce = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      let pendingAce = await this.chef.pendingAce(0, this.alice.address)
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef.updatePool(0))
            ...
            .withArgs(0, (await this.chef.poolInfo(0)).lastRewardTime,
              ...
              (await this.chef.poolInfo(0)).accAcePerShare)
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of ACE and reward", async function () {
        ...
        let timestamp = (await ethers.provider.getBlock(log.blockNumber)).timestamp
        let expectedAce = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
        expect((await this.chef.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedAce)
