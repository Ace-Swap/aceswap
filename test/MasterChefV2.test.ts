...
describe("MasterChefV2", function () {
  before(async function () {
    await prepare(this, ['MasterChef', 'AceToken', 'ERC20Mock', 'MasterChefV2', 'RewarderMock', 'RewarderBrokenMock'])
    await deploy(this, [
  ...
  beforeEach(async function () {
    ...
    await deploy(this,
      ...
      ['chef', this.MasterChef, [this.ace.address, this.bob.address, getBigNumber(100), "0", "0"]]
    ])
    ...
    await deploy(this, [
        ['chef2', this.MasterChefV2, [this.chef.address, this.ace.address, 1]],
  ...
  describe("PendingAce", function() {
    it("PendingAce should equal ExpectedAce", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      ...
      await advanceBlock()
      let expectedAce = getBigNumber(100).mul(log2.blockNumber + 1 - log.blockNumber).div(2)
      let pendingAce = await this.chef2.pendingAce(0, this.alice.address)
      expect(pendingAce).to.be.equal(expectedAce)
    })
    it("When block is lastRewardBlock", async function () {
      ...
      let log2 = await this.chef2.updatePool(0)
      let expectedAce = getBigNumber(100).mul(log2.blockNumber - log.blockNumber).div(2)
      let pendingAce = await this.chef2.pendingAce(0, this.alice.address)
      expect(pendingAce).to.be.equal(expectedAce)
    })
  })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
            ...
            .withArgs(0, (await this.chef2.poolInfo(0)).lastRewardBlock,
              ...
              (await this.chef2.poolInfo(0)).accAcePerShare)
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of ACE and reward", async function () {
        ...
        let expectedAce = getBigNumber(100).mul(log2.blockNumber - log.blockNumber).div(2)
        expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedAce)
        await this.chef2.harvest(0, this.alice.address)
        expect(await this.ace.balanceOf(this.alice.address)).to.be.equal(await this.r.balanceOf(this.alice.address)).to.be.equal(expectedAce)
    })
    ...
    it("Harvest for ACE-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedAce)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.ace.balanceOf(this.alice.address)).to.be.equal(expectedAce)
    })
  })
